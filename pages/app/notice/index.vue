<template>
	<view>
		...
		<uv-navbar border placeholder auto-back title="系统通知"></uv-navbar>
...
import { ref } from 'vue';
import { pageNotice } from '@/api/partyaffairs/notice.js';
import NoticeCard from './card.vue';

const toastRef = ref();
const current = ref(0);
const tabList = ref(['全部', '未读']);
const params = ref({ page: 1, page_size: 10, not_read: null });
const noticeList = ref([]);
const loadStatus = ref('loading');

const fetchData = () => {
	pageNotice(params.value).then((res) => {
