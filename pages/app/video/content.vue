<template>
	...
	<view class="header" v-if="current && !isPdf(current.resource?.url)">
		<video
			id="video"
			object-fit="fill"
			direction="90"
...
import { pageVideoContent, reportProcess } from '@/api/partyaffairs/video.js';
...
const handlePlay = (item) => {
	if (isPdf(item.resource?.url)) {
		uni.navigateTo({
			url: './pdf?src=' + item.resource?.url + '&id=' + item.id
		});
		return;
	}
